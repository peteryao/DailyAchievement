...
def index(request):
    ...
    return render_to_response('gamify/index.html', {
...
def profile(request, user_id):
    ...
    user_groups = UserGroup.objects.filter(user_id=request.user.id)
    return render_to_response('gamify/profile.html', {
        "current_user": request.user,
        'groups': user_groups,
        'profile_user': user,
...
def quest(request):
    ...
    print user_quests
    user_groups = UserGroup.objects.filter(user_id=request.user.id)
    return render_to_response('gamify/quests.html', {
        'current_user': request.user,
...
def group(request, group_id):
    ...
    user_groups = UserGroup.objects.filter(user_id=request.user.id)
    return render_to_response('gamify/group.html', {
        'current_user': request.user,
        'groups': user_groups,
        "group": group,
        "members": sort,
...
def leaderboard(request):
    ...
    user_groups = UserGroup.objects.filter(user_id=request.user.id)
    return render_to_response('gamify/leaderboard.html', {
        'current_user': request.user,
        'groups': user_groups,
        "board": board,
        "quests": quests,
...
def quest_page(request, quest_id):
    ...
    user_groups = UserGroup.objects.filter(user_id=request.user.id)
    return render_to_response('gamify/quest_page.html', {
        'current_user': request.user,
        'groups': user_groups,
        'quest': quest,
...
def about(request):
    ...
    user_groups = UserGroup.objects.filter(user_id=request.user.id)
    return render_to_response('gamify/about.html', {
        'current_user': request.user,
        'groups': user_groups,
        }, context_instance=RequestContext(request))
...
def current_quest(request):
    ...
    user_groups = UserGroup.objects.filter(user_id=request.user.id)
    return render_to_response('gamify/current_quest.html', {
        'current_user': user,
...
def add_group(request):
    ...
    return render_to_response('gamify/add_group.html', {
        "current_user": user,
        'groups': user_groups,
